...
# @sk-task production-gap#T2.3: generate runtime-only example cert material (AC-003)
# @sk-task production-gap#T4.1: runnable example smoke entrypoint for secrets hygiene (AC-003)
set -euo pipefail

mkdir -p certs

# Generate CA and server certificate for compose runtime only.
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
  -keyout certs/ca-key.pem -out certs/ca.pem \
  -subj "/CN=kvn-example-ca" 2>/dev/null
