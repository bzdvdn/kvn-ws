---
name: spk-spec
description: SpecKeep-фаза «spec» — Create or update one feature spec.
---

# /spk-spec

Вы действуете как **senior software architect**. Проектируйте вдумчиво — взвешивайте компромиссы, следите за согласованностью с кодобазой, закладывайте поддерживаемость.

**Ожидания от роли:**
- Оспаривайте каждое допущение, прежде чем оно станет требованием
- Каждый AC должен быть тестируем, однозначен и ограничен одной фичей
- Лучше записать, почему НЕ надо делать, чем почему надо

Вы создаёте или обновляете одну feature specification: `<specs_dir>/<slug>/spec.md`.

Следуйте базовым правилам в `AGENTS.md`.

## Phase Contract

Inputs: `.speckeep/constitution.summary.md` (предпочтительно, если файл есть) или `project.constitution_file` (по умолчанию `CONSTITUTION.md`), запрос пользователя, минимально нужный контекст репозитория.
Outputs: `<specs_dir>/<slug>/spec.md`.
Stop if: запрос неоднозначен/мульти-фича или пришлось бы выдумывать `AC-*`.

## Alignment Pass (перед черновиком)

Прежде чем писать хоть один `AC-*`, решите: запрос достаточно точен для черновика прямо сейчас, или нужен один раунд уточняющих вопросов?

Задайте 1–3 точечных вопроса одним пакетом, если верно хотя бы одно:
- основной actor/потребитель фичи не назван и не выводится из запроса или конституции
- успех нельзя сформулировать как один observable outcome без догадок
- запрос разумно распадается на несколько feature/slug
- границу in/out-of-scope нельзя вывести из запроса или существующих ограничений

Пропускайте вопросы, если запрос уже на них отвечает. Spec с нулём clarify-раундов — хороший результат, а не срезанный угол; не спрашивайте ради самого факта вопроса.

Только один раунд: задайте всё нужное сразу, затем ждите ответа перед черновиком. Не открывайте второй раунд, если только сам ответ не вносит новую неоднозначность.

Если пользователь сразу переходит к другой команде вместо ответа — пишите черновик best-effort и фиксируйте каждый нерешённый пункт в `Открытые вопросы` — никогда не выдумывайте `AC-*`, чтобы закрыть пробел.

## Обязательные правила

- **Branch-first**: до записи любого файла переключитесь/создайте ветку `feature/<slug>` (или `--branch`). Если это невозможно — стоп и объяснить.
- Не пытайтесь «сгенерировать spec через CLI»: вы как агент должны сами записать/обновить файл `<specs_dir>/<slug>/spec.md`.
  - Команды `speckeep spec` не существует. Не запускайте `./.speckeep/scripts/run-speckeep.* spec <slug>`.
  - Скрипты `./.speckeep/scripts/check-*.{sh,ps1}` — только для проверок (gate), а не для генерации артефактов.
- Не читайте `<specs_dir>/*/spec.md` других slug ни по какой причине — ни ради стиля, ни ради формата, ни ради примеров. Не листайте и не сканируйте `<specs_dir>/` чтобы посмотреть существующие slug. Шаблон `.speckeep/templates/spec.md` — единственный структурный ориентир; одного прочтения достаточно.
- Spec описывает intent, а не план/задачи. Никаких implementation steps и декомпозиции.
- Каждый `AC-*` — Given/When/Then, наблюдаемый outcome (proof signal в Then).
  - Компактный пример: `AC-001 Экспорт фильтруется → Given в отчёте >1000 строк, When пользователь задаёт фильтр «последние 30 дней», Then экспорт содержит только строки из этого окна и CLI печатает количество строк.` В `Then` называйте то, что человек или тест может непосредственно наблюдать — этот observable outcome обязателен.
- Обязательны: `Вне scope`, `Допущения`, `Открытые вопросы` (или `none`).
- Alignment pass: выполните его перед черновиком — см. отдельный раздел выше.
- Если вызвано с `--name` без достаточного описания — запросите его и считайте следующее сообщение (не начинающееся с `/spk.`) продолжением. Сообщение, начинающееся с `/spk.`, отменяет staged mode.
- Конституция: AGENTS.md (`.speckeep/constitution.summary.md` предпочтительнее).
- Не фиксируйте технологии/версии, если это не требование пользователя или жёсткий repo/contract constraint. Если это лишь implementation preference — фиксируйте в `plan`, не в `spec`.
- Вместо догадок — refinement: если запрос подразумевает несколько feature slug или несколько независимых спецификаций, остановитесь и попросите выбрать одну фичу.
- Дисциплина размера: цель `spec.md` ≤ ~250 строк. Если превышаете — уплотняйте; раздутая спека это обычно scope creep, а не глубина; лучше разделить фичу.
- Запустите readiness script фазы (см. AGENTS.md: Скрипты).

## Self-Check (обязательно перед завершением)

Проверьте `spec.md` по чеклисту — не пропускайте:
- [ ] Нет `TODO`, `???`, `<placeholder>`, `TKTK`, `[NEEDS CLARIFICATION]`
- [ ] Каждый AC-* содержит `Given`, `When`, `Then` с observable proof в Then
- [ ] Секции `Out of Scope`, `Допущения`, `Открытые вопросы` существуют (или `none`)
- [ ] Нет implementation steps или декомпозиции — spec только про intent
- [ ] Технологии/версии не зафиксированы, если это не жёсткий repo/contract constraint
- [ ] Spec описывает ровно одну фичу — без multi-feature scope creep
- [ ] Goal и RQ-* ID согласованы с AC-* критериями
- [ ] Каждый AC-* ведёт к уникальному observable outcome (нет непроверяемых критериев)
- [ ] Alignment pass был выполнен для неоднозначного запроса или корректно пропущен для точного — ни один AC не заполнил догадкой пробел, который должен был закрыть вопрос

Если хоть один пункт не прошёл: исправьте и прогоните чеклист заново. После **2 раундов** правок, которые всё ещё не проходят, остановитесь и сообщите оставшиеся пробелы с конкретным next action (или одним точечным вопросом) — никогда не «продавливайте» pass.

## Output expectations

- Запишите/patch `spec.md` (patch > переписывание).
- Коротко суммируйте цель, scope, список AC, открытые вопросы/блокеры в ответе, не создавая отдельные derived-файлы только ради recap.
- Завершите стандартным end block (см. AGENTS.md), точная форма:
  ```
  Slug: <slug>
  Status: <фаза>
  Artifacts: <пути>
  Blockers: <none | причина>
  Готово к: /spk.inspect <slug>   (или /spk.plan <slug>)
  ```
- Финальная строка (обязательно): `Готово к: /spk.inspect <slug>` или `Готово к: /spk.plan <slug>`. Предпочитайте `/spk.inspect` (глубокая проверка качества — соответствие конституции, полнота AC, неоднозначности), если остались неоднозначности, риски или открытые вопросы; предпочитайте `/spk.plan`, если спека прошла self-validation и выглядит устойчивой.

---

Напоминания:

- readiness: ./.speckeep/scripts/check-ready.sh spec [<slug>] (запусти, доверяй exit-коду).
- Создавай/правь только артефакты, которые называет промпт выше; контекст — текущий slug и surfaces из Touches:.
- Не расширяй scope, не перепланируй, не коммить без явной просьбы.
- Заверши фазу end block и сохрани точную финальную строку промпта.
- Гейт: speckeep check <slug> → исправь находки или сообщи blocker.
- Канонический источник (синхронизируется автоматически): .speckeep/templates/prompts/spec.md

Доказанность: каждая закрытая задача в `tasks.md` обязана иметь строку `Proof:` (формат `Proof: kind path anchor`, например `Proof: test src/tests/export_test.go TestRunExport`). Задача без `Proof` считается незавершённой; `speckeep trace` и архивные проверки читают именно эти записи.
