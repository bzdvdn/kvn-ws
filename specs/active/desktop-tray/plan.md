...
| `src/cmd/desktop/app_darwin.go` | модификация | legacyRun (без tray — отложено на отдельную спеку) |
...
| ~~`src/cmd/desktop/tray_darwin.go`~~ | ~~новая~~ | ~~CGo NSStatusBar impl (`.mm` companion)~~ отложено |
...
- Для macOS: ~~`tray_darwin.mm` — Objective-C++ файл рядом с `.go` в том же пакете (CGo).~~ отложено
...
- **macOS CGo + extern "C"** — сложность сборки Objective-C кода через CGo. macOS tray отложен на отдельную спеку — CI-доступа к macOS runner для отладки нет. `app_darwin.go` использует legacy-путь (webview без трея).
