...
Inputs: spec `specs/active/quic-transport/spec.md`, кодовая база kvn-ws.
...
1. `go test ./src/internal/transport/quic/...` — unit
2. iperf через туннель на канале с `tc netem delay 60ms loss 1%` > 5 Mbps
...
| `src/cmd/server/main.go` | QUIC listener рядом с TCP |
