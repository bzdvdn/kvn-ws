...
- **`multi_channel: true` включён по умолчанию в поставляемых конфигах** — `configs/client.yaml`, `examples/client.yaml`, генераторы `install-client.sh`/`install-client.ps1`. Избавляет десктоп-клиентов от фризов/робовойса в звонках с видео (UDP-медиа уходит во второй WS-канал, без head-of-line blocking).

### Fixed

- **Android: краш per-app filter на старых Android (API < 33)** — `PackageManager.ApplicationInfoFlags.of(0)` (доступен только с API 33) заменён на гейт по `Build.VERSION.SDK_INT` со старым `getInstalledApplications(0)`.
- **Android: NPE при импорте QR** — поля `auth`/`tls`/`crypto` в QR-модели `WebConfig` сделаны nullable, `webToAndroidConfig` переведён на null-safe доступы; конфиг с пустым `server` отбраковывается как невалидный.
- **Android: дублирование сервера при добавлении/сканировании QR** — `MainViewModel.addServer`/`duplicateServer` слепо добавляли новую запись в список без проверки на совпадение имени, а `QrCodeAnalyzer` не останавливался после первого успешного распознавания QR и мог вызвать колбэк несколько раз подряд на соседних кадрах. `addServer`/`duplicateServer` теперь делают upsert по имени, а анализатор прекращает декодирование сразу после первого результата (флаг `found`).
- **kvn-web: список серверов показывал не тот сервер как активный/подключённый** — `activeServer` в контексте одновременно означал «сервер открыт в форме редактирования» и «сервер, к которому подключены»: клик по другому серверу в списке (просто для просмотра настроек) тут же красил его как «connected», хотя трафик шёл через ранее подключённый сервер. Состояние разделено на `activeServer` (открыт в форме) и `connectedServer` (реально подключён — обновляется только при успешном Connect и при начальной загрузке страницы).
- **kvn-web: «Copy config» мог скопировать устаревшую версию настроек** — кнопка копирования в строке списка брала данные из последнего загруженного снэпшота серверов, а не из текущей (возможно несохранённой) формы редактирования. Для сервера, открытого в форме, теперь копируются актуальные данные формы.
- **kvn-web: ошибки добавления/импорта/удаления сервера проглатывались** — `addServer`/`doImport`/`deleteServer` не проверяли HTTP-статус ответа: при конфликте имени (`409`) или сетевой ошибке UI всё равно показывал «Server created»/«Imported». Ошибки бэкенда теперь корректно ловятся и отображаются тостом.
...
- **Android: спам уведомлением каждые 2с** — цикл `notificationUpdateJob` в `handleFrame()` дёргал `startForeground()` каждые 2 секунды. В комбинации с `IMPORTANCE_HIGH` это вызывало постоянные звуки/вибро. Исправлено: цикл удалён (нотификация и так обновляется через `onConnectionStateChange`), важность канала понижена до `IMPORTANCE_LOW`.
...
- **macOS TUN** — полноценная поддержка TUN на macOS: создание `utun`, read/write data path, SetIP/SetMTU через `ifconfig`, SetGateway/RemoveGateway/AddExcludeRoute через `route`, SaveDefaultRoute, CleanupExcludeRoutes, CleanupStaleExcludeRoutes. `tun_supported = true` на darwin. LaunchDaemon (`com.kvn.tun.plist`) и LaunchAgent (`com.kvn.web.plist`) для автозапуска.
- **DNS routing (кросс-платформенный)** — DNS-регистрация на TUN-интерфейсе: Linux (resolv.conf/systemd-resolved), Windows (winipcfg.LUID.SetDNS), macOS (networksetup). Platform API: `setupDNS/applyDNS/restoreDNS` в `bootstrap/client/`. `CleanupStaleDNS` на всех платформах. DNS proxy core (`Server`, `Run`, `forwarding`) платформенно-независимый.
- **DNS routing: exclude/include домены** — поддержка точных (exact) доменов (`example.com`) в дополнение к suffix (`.ru`). `MatchDomain` проверяет exact-домены через отдельную карту `exactDomains`. DNS proxy стартует при любых доменах, не только suffix. Авто-включение `dns_routing.enabled` при добавлении доменов на фронтенде и бэкенде (`mergeConfig`, `NewFromConfig`).
...
- **DNS routing bugfix** — `SetClientDefaults` добавлен в save-хендлеры (`handleSaveGlobalConfig`, `handleUpdateServer`, `handleCreateServer`), чтобы `DNSRouting.TTL=60` проставлялся до YAML marshal (иначе `omitempty` опускал TTL при сериализации). Backend: `mergeConfig` и `NewFromConfig` теперь авто-включают `DNSRouting.Enabled=true` при наличии exclude/include доменов.
