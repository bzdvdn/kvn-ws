...
Разрешите форвардинг трафика через TUN-интерфейс в nftables:
...
Все настройки клиента доступны через веб-интерфейс. **kvn-web** поддерживает управление несколькими серверами:
- **Глобальные настройки** (mode, proxy listen, log level, MTU, маршрутизация по умолчанию) применяются ко всем серверам
- **Per-server настройки** (URL сервера, токен, TLS, переопределения маршрутизации, обфускация, шифрование) хранятся индивидуально
- **Селектор серверов** позволяет переключаться между конфигурациями; несохранённые изменения вызывают диалог подтверждения

| Раздел | Поля |
|--------|------|
| Servers | Добавление/удаление/импорт серверов, экспорт/QR конфига |
